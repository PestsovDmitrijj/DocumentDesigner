...
class ParagraphStyle extends Style {
	...
	public function __construct($name, $parentStyleName, $masterPageName) {
    if (empty($name)) {
      $this->name = 'paragraphstyle'.rand(100, 9999999);
    }
		parent::__construct($name);
		$this->styleElement->setAttribute('style:family', 'paragraph');
		if ($parentStyleName != null) {
			$this->styleElement->setAttribute('style:parent-style-name', $parentStyleName);
		}
	...
	function setBreakAfter($breakAfter) {
		switch ($breakAfter) {
			...
			// hand
			case StyleConstants::STYLE:
				$breakAfter = 'style';break;
			//hand
			default:
