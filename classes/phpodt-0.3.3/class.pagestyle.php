...
class PageStyle {
	 ...
	 */
	 ...
	 // function __construct($name) {
		...
		// $pageLayoutStyleElement->setAttribute('style:name', 'Standard');
	 ...
	 // }
	 // function setStyle($layoutName, $masterPageName, $nextStyleName) {
		// $pageLayoutStyleElement = $this->styleDocument->createElement('style:page-layout');
		// $this->pageLayoutProperties = $this->styleDocument->createElement('style:page-layout-properties');
		
		// $this->styleDocument->getElementsByTagName('office:automatic-styles')->item(0)->appendChild($pageLayoutStyleElement);
		// $this->masterStyleElement = $this->styleDocument->createElement('style:master-page');
		// $this->masterStyleElement->setAttribute('style:name', $masterPageName);
		// $this->masterStyleElement->setAttribute('style:page-layout-name', $layoutName);
		// if ($nextStyleName != null) {
			// $this->masterStyleElement->setAttribute('style:next-style-name', $nextStyleName);
		// }
		// $this->styleDocument->getElementsByTagName('office:master-styles')->item(0)->appendChild($this->masterStyleElement);
		// //$this->setHorizontalMargin('2cm', '2cm');
	 // }
	 
	 function __construct($name, $masterPageName, $orientation, $nextStyleName) {
		$this->styleDocument = ODT::getInstance()->getStyleDocument();
		$this->name = $name;
		$pageLayoutStyleElement = $this->styleDocument->createElement('style:page-layout');
		$this->pageLayoutProperties = $this->styleDocument->createElement('style:page-layout-properties');
		...
		//===========================================
		// Используем имя master-page, на котором будет макет
		//===========================================
		...
		//===========================================
		// Дописываем  стиль, следующий за заданным
		//===========================================
	 ...
	 // function addMasterPage($name, $layoutName) {
		// $this->masterStyleElement = $this->styleDocument->createElement('style:master-page');
		// $this->masterStyleElement->setAttribute('style:name', $name);
		// $this->masterStyleElement->setAttribute('style:page-layout-name', $layoutName);
		// $this->pageLayoutProperties->setAttribute('style:print-orientation', 'landscape');
	// }
	...
	// function addMasterPage($name, $layOutName, $nextStyleName) {
		// $this->styleDocument = ODT::getInstance()->getStyleDocument();
		// $this->masterStyleElement = $this->styleDocument->createElement('style:master-page');
		// $this->masterStyleElement->setAttribute('style:name', $masterPageName);
		// $this->masterStyleElement->setAttribute('style:page-layout-name', $layOutName);
	...
	// function addPageLayout($name, $orientation) {
		// //$this->styleDocument = ODT::getInstance()->getStyleDocument();
		// $pageLayoutStyleElement = $this->styleDocument->createElement('style:page-layout');
		// $this->pageLayoutProperties = $this->styleDocument->createElement('style:page-layout-properties');
		// switch ($orientation) {
	...
	function setHeadFootContent($element, $content, $paragraphStyles = null) {
		...
		if (!($content instanceof Table) && $content == StyleConstants::PAGE_NUMBER) {
		...
		} else if(!($content instanceof Table) && $content == StyleConstants::CURRENT_DATE) {
